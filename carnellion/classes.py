...
class USBTransfer:
    ...
    def __init__(
        ...
        self.free = libusb1.libusb_free_transfer
    ...
    def setBuffer(self, buffer_or_len):
        ...
        self.libcancel = libusb1.libusb_cancel_transfer
        # pylint: enable=unused-private-member
        self.__transfer_py_buffer = transfer_py_buffer
...
class USBContext:
    ...
    def openByVendorIDAndProductID(
            self, vendor_id, product_id,
            skip_on_access_error=False, skip_on_error=False,auto=True):
        """
        Get the first USB device matching given vendor and product ids.
    ...
    @_validContext
    def handleEventsTimeoutCompleted(self,tv=0,completed=0):
        """
        Handle any pending events by checking if timeouts have expired 
        and by checking the set of file descriptors for activity. 
        If the completed argument is not equal to NULL, 
        this function will loop until a transfer completion callback sets the variable 
        pointed to by the completed argument to non-zero. 
        If the tv argument is not equal to NULL, 
        this function will return LIBUSB_ERROR_TIMEOUT after the given timeout. 
        ...
        tv_s = int(tv)
        real_tv = libusb1.timeval(tv_s, int((tv - tv_s) * 1000000))
        return libusb1.libusb_handle_events_timeout_completed(self.__context_p,byref(real_tv),c_int(completed))
